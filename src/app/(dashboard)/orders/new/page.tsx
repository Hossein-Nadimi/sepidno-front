...
export default function NewOrderPage() {
  ...
  const createMutation = useMutation({
    mutationFn: () => {
      ...
      // Send the Jalali date string directly (e.g. "1404/05/03").
      // The backend converts it to a UTC Date via jalaliDayRange(), which
      // is timezone-safe. This avoids any moment-jalaali UTC vs local issues.
            ...
            <Card>
              <CardContent className="space-y-4 pt-6">
                {/* Services for selected garment — 2-column grid */}
                  ...
                  {/* Quantity / Meterage — stacked on mobile, side-by-side on desktop.
                      For per-meter garments (e.g. پرده), the field accepts decimals. */}
            ...
            <CardContent className="space-y-3">
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  ...
                  {showDatePicker && (
                    <>
                      {/* Mobile backdrop so taps outside the calendar close it */}
                      <div
                        className="fixed inset-0 z-40 bg-black/30 sm:hidden"
                        onClick={() => setShowDatePicker(false)}
                      />
                      <div className="fixed inset-x-2 bottom-2 z-50 sm:absolute sm:inset-x-0 sm:bottom-auto sm:mt-1 sm:left-0 sm:right-0">
                        <JalaliDatePicker
                          value={deliveryDate}
                          onChange={(v) => { setDeliveryDate(v); setShowDatePicker(false); }}
